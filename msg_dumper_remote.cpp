...
public:
	...
	~RemoteServerCfg()
	{
		if (sockfd != 0)
		{
			WRITE_DEBUG("Close the connection: %s", ip);
			close(sockfd);
			sockfd = -1;
		}
	}
...
unsigned short MsgDumperRemote::open_device(){return MSG_DUMPER_SUCCESS;}

unsigned short MsgDumperRemote::close_device(){return MSG_DUMPER_SUCCESS;}
...
// Try to connect to the server
	...
	while(tok)
	{
		...
		WRITE_DEBUG("Try to connect to the server: %s......", tok);

// Initialize the client socket
		pRemoteServerCfg->sockfd = socket(AF_INET, SOCK_STREAM, 0);
		if (pRemoteServerCfg->sockfd == -1)
		{
			WRITE_ERROR("Fail to create a client socket: client_sockfd");
			ret = MSG_DUMPER_FAILURE_SOCKET;
			break;
		}

// Connect
		WRITE_DEBUG("Client connect to server[%s]", pRemoteServerCfg->ip);
//		int client_len;
		...
		WRITE_DEBUG("Try to connect to the server: %s...... Successfully", pRemoteServerCfg->ip);
...
{
	WRITE_DEBUG("DeInitialize the MsgDumperRemote object......");
...
// Write the message into the log file
	...
	for(int j = 0 ; j < server_socket_vector_size ; j++)
	{
		...
		WRITE_DEBUG("Write the message[%s] to remote server [%s]", msg_cfg->to_string(), remote_server_cfg->ip);
